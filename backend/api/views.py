...
class AdminSEOReportView(APIView):
    """Serve the SEO report JSON generated by the management command."""
    permission_classes = [IsAuthenticated, AdminDashboardPermission]

    def get(self, request):
        import json as _json
        report_path = os.path.join(settings.BASE_DIR, 'logs', 'seo_report.json')
        if not os.path.exists(report_path):
            return Response({'error': 'No report found. Run: python manage.py generate_seo_reports'}, status=404)
        with open(report_path, 'r', encoding='utf-8') as f:
            data = _json.load(f)
        return Response(data)

    def post(self, request):
        """Trigger a fresh report generation."""
        from django.core.management import call_command
        try:
            call_command('generate_seo_reports')
            import json as _json
            report_path = os.path.join(settings.BASE_DIR, 'logs', 'seo_report.json')
            with open(report_path, 'r', encoding='utf-8') as f:
                data = _json.load(f)
